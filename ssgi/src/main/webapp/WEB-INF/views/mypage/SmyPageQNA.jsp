...
<script>
	function selChange() {
		...
		//location.href="myPageQNA?nowPage=${paging.nowPage}&cntPerPage="+sel;
		 if(sel == 5) {
			location.href="SmyPage";
		}else {
			location.href="SmyPageQNA?nowPage=${paging.nowPage}&cntPerPage="+sel;
		}  
	}
</script>
...
<input type="hidden" name="USER_ID"  class="form-control"  value="${user_id }">
		...
		<form action="myQNADelete" method="post">
			<table class="table ">
				...
				<tbody>
					<tr>
						...
						<td><a href="SmyQNADetail?AS_QNA_NUM=${dto.AS_QNA_NUM}">${dto.AS_QNA_TITLE}</a></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
						...
						<c:choose>
						<c:when test="${dto.AS_QNA_READ eq '답변완료'}"><a  href="SmyQNADetail?AS_QNA_NUM=${dto.AS_QNA_NUM}" style="color:green">답변완료</a></c:when>
						</c:choose>
						</td>
						<td><a href="SmyQNADelete?AS_QNA_NUM=${dto.AS_QNA_NUM}" onclick="return SQNADelete()">X</a></td>
					</tr>
			...
			<hr>
			<form action="SmyQNAWrite" method="get" >
			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
	...
	<div style="display: block; text-align: center;" >		
		...
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a id="page" href="SmyPageQNA?nowPage=${p  }&cntPerPage=${paging.cntPerPage}" >${p }</a>
				</c:when>
		...
		<c:if test="${paging.endPage != paging.lastPage}">
			<a  href="SmyPageQNA?nowPage=${paging.endPage + 1 }&cntPerPage=${paging.cntPerPage}">&gt;&gt;</a>
		</c:if>
	</div>
